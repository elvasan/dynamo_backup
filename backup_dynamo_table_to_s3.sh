...
provisionCluster() {
  if [ $new_read_iops -gt $read_capacity ]; then
    ...
    cluster_id=$(aws emr create-cluster --auto-terminate \
                 --steps Type=HIVE,Name='ddb-backup',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/$table-backup.hql] Type=HIVE,Name='s3-write',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/${table}-s3-write.hql] \
                 --applications Name=Hadoop Name=Hive Name=Pig Name=Hue \
                 --ec2-attributes '{"KeyName":"data","InstanceProfile":"EMR_EC2_DefaultRole","AvailabilityZone":"'${availability_zone}'","EmrManagedSlaveSecurityGroup":"sg-7b8a81ec","EmrManagedMasterSecurityGroup":"sg-faf8f36d"}' \
                 --service-role EMR_DefaultRole \
                 --enable-debugging \
...
addStepsToCluster() {
  if [ $new_read_iops -gt $read_capacity ]; then
    total_steps=4
    step_ids=($(aws emr add-steps --cluster-id ${cluster_id} \
                --steps Type=CUSTOM_JAR,Name=CustomJAR,ActionOnFailure=CANCEL_AND_WAIT,Jar=s3://elasticmapreduce/libs/script-runner/script-runner.jar,Args=["s3://${S3_BUCKET}/change_throughput_$table.sh","$new_read_iops"] Type=HIVE,Name='ddb-backup',ActionOnFailure=CANCEL_AND_WAIT,Args=[-f,s3://${S3_BUCKET}/$table-backup.hql] Type=CUSTOM_JAR,Name=CustomJAR,ActionOnFailure=CANCEL_AND_WAIT,Jar=s3://elasticmapreduce/libs/script-runner/script-runner.jar,Args=["s3://${S3_BUCKET}/change_throughput_$table.sh","$read_capacity"] Type=HIVE,Name='s3-write',ActionOnFailure=CANCEL_AND_WAIT,Args=[-f,s3://${S3_BUCKET}/${table}-s3-write.hql] | \
                jq -r .StepIds[]))
  else
    total_steps=2
    step_ids=($(aws emr add-steps --cluster-id ${cluster_id} \
                --steps Type=HIVE,Name='ddb-backup',ActionOnFailure=CANCEL_AND_WAIT,Args=[-f,s3://${S3_BUCKET}/$table-backup.hql] Type=HIVE,Name='s3-write',ActionOnFailure=CANCEL_AND_WAIT,Args=[-f,s3://${S3_BUCKET}/${table}-s3-write.hql] | \
                jq -r .StepIds[]))
  fi
}
