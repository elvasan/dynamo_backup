...
provisionCluster() {
  if [ $new_read_iops -gt $read_capacity ]; then
    cluster_id=$(aws emr create-cluster --auto-terminate --steps Type=CUSTOM_JAR,Name=CustomJAR,ActionOnFailure=CANCEL_AND_WAIT,Jar=s3://elasticmapreduce/libs/script-runner/script-runner.jar,Args=["s3://${S3_BUCKET}/change_throughput_$table.sh","$new_read_iops"] Type=HIVE,Name='ddb-backup',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/$table-backup.hql] Type=CUSTOM_JAR,Name=CustomJAR,ActionOnFailure=CANCEL_AND_WAIT,Jar=s3://elasticmapreduce/libs/script-runner/script-runner.jar,Args=["s3://${S3_BUCKET}/change_throughput_$table.sh","$read_capacity"] Type=HIVE,Name='s3-write',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/${table}-s3-write.hql] --applications Name=Hadoop Name=Hive Name=Pig Name=Hue --ec2-attributes '{"KeyName":"data","InstanceProfile":"EMR_EC2_DefaultRole","AvailabilityZone":"'${availability_zone}'","EmrManagedSlaveSecurityGroup":"sg-5632583e","EmrManagedMasterSecurityGroup":"sg-5432583c"}' --service-role EMR_DefaultRole --enable-debugging --release-label emr-5.4.0 --log-uri 's3n://aws-logs-298785453590-us-east-1/elasticmapreduce/' --name "$table backup" --instance-groups '[{"InstanceCount":1,"InstanceGroupType":"MASTER","InstanceType":"m3.xlarge","Name":"Master instance group - 1"},{"InstanceCount":4,"BidPrice":"'${spot_bid_price}'","InstanceGroupType":"CORE","InstanceType":"'${core_instance_type}'","Name":"Core instance group - 5"}]' --region us-east-1 | jq -r .ClusterId)
  else
    cluster_id=$(aws emr create-cluster --auto-terminate --steps Type=HIVE,Name='ddb-backup',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/$table-backup.hql] Type=HIVE,Name='s3-write',ActionOnFailure=CONTINUE,ActionOnFailure=TERMINATE_CLUSTER,Args=[-f,s3://${S3_BUCKET}/${table}-s3-write.hql] --applications Name=Hadoop Name=Hive Name=Pig Name=Hue --ec2-attributes '{"KeyName":"data","InstanceProfile":"EMR_EC2_DefaultRole","AvailabilityZone":"'${availability_zone}'","EmrManagedSlaveSecurityGroup":"sg-5632583e","EmrManagedMasterSecurityGroup":"sg-5432583c"}' --service-role EMR_DefaultRole --enable-debugging --release-label emr-5.4.0 --log-uri 's3n://aws-logs-298785453590-us-east-1/elasticmapreduce/' --name "$table backup" --instance-groups '[{"InstanceCount":1,"InstanceGroupType":"MASTER","InstanceType":"m3.xlarge","Name":"Master instance group - 1"},{"InstanceCount":4,"BidPrice":"'${spot_bid_price}'","InstanceGroupType":"CORE","InstanceType":"'${core_instance_type}'","Name":"Core instance group - 5"}]' --region us-east-1 | jq -r .ClusterId)
  fi
}
